...
library(dplyr)
# Ensure only numeric columns are used for richness calculation
numeric_columns <- sapply(data, is.numeric)
data$richness <- rowSums(data[, numeric_columns], na.rm = TRUE)

# Create a new column to group sections
data <- data %>%
  mutate(section_group = case_when(
    section %in% 1:3 ~ "Group 1 (Sections 1-3)",
    section %in% 4:5 ~ "Group 2 (Sections 4-5)",
    section %in% 6:8 ~ "Group 3 (Sections 6-8)",
    TRUE ~ "Other"  # Catch-all for any unexpected sections
  ))

# Summarize richness by the new section groups
